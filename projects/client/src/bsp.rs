...
pub fn load_bsp(filename: &str) -> Bsp
{
	...
	let tex_infos= read_texinfo(header.tex_info, &mut reader, &mut buf, &mips);
	let surfaces = read_faces(header.faces, &mut reader, &mut buf, &tex_infos, &mips, &vertexes, &surf_edges, &edges);
